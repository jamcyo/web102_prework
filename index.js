...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `<img class = game-img src = ${games[i].img} />
                              ...
                              <div>Goal: $${games[i].goal.toLocaleString('en-US')}</div>
                              <div>Pledged: $${games[i].pledged.toLocaleString('en-US')}</div>`;
...
function filterUnfundedOnly() {
    ...
    let GoalsUnmet = GAMES_JSON.filter((game) => {
      return game.goal > game.pledged;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(GoalsUnmet);
}
...
const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length != 1 ? " games" : " game"}.
                    Currently, ${numUnfundedGames} ${numUnfundedGames != 1 ? " games remain" : "game remains"} unfunded. We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
let descr = document.createElement("p")
descr.innerHTML = displayStr
descriptionContainer.appendChild(descr);
